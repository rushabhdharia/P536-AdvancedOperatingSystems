...
void *Find(void *ptr)
{
    ...
    while ((read = getline(&line, &len, fileinput)) != -1) //https://linux.die.net/man/3/getline
    {
    	if(strstr(line, word))
...
int main(int argc, char **argv)
{
	...
	{
		n = 5;
		strcpy(filename, "newfile.txt"); //to save the input from the pipe
		strcpy(word, argv[1]); 
	}
	else if(argc == 3)
	{
	...
	for (t = 0; t < n; t++)
	{
	   ...
	   sprintf(filename2[t], "filename_%d.txt", t); 
